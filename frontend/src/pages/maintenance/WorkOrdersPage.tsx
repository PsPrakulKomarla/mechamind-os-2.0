...
export const WorkOrdersPage = () => {
  ...
  const displayWorkOrders = useMemo(() => {
    if (workOrders && Array.isArray(workOrders) && workOrders.length > 0) return workOrders;
    if (workOrders?.items && workOrders.items.length > 0) return workOrders.items;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 h-full flex flex-col">
      ...
      <div className="flex-1 mt-2">
        <WorkOrderBoard workOrders={displayWorkOrders as any} isLoading={isLoading} />
      </div>
